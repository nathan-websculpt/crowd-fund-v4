      ...
      describe("Handling donations", function () {
        ...
        describe("Force-Ending Fund Runs ", function () {
          it("(2-sig wallet)...Alice proposes to pay John 0.25 Ethers", async function () {
            ...
            const fundRunID = 1;
            const proposalID = 0;
            const endFundRun_Tx = await crowdFund.connect(alice).forceEnd(fundRunID);
            endFundRun_Tx.wait();
          ...
          it("(3-sig wallet)...The Chan-Chan Man proposes to pay Bob 0.5 Ethers", async function () {
            ...
            const fundRunID = 2;
            const proposalID = 2;
            const endFundRun_Tx = await crowdFund.connect(bob).forceEnd(fundRunID);
            endFundRun_Tx.wait();
            const transferAmount = parseEther("0.5");
            const reason =
              "The Chan-Chan Man proposes to pay Bob (who is not an owner) 0.5 Ethers for web design services";
            const multisigReq = {
          ...
          it("(3-sig wallet)...Ross proposes to pay Alice 0.5 Ethers", async function () {
            ...
            const transferAmount = parseEther("0.5");
            const reason =
              "Ross proposes to pay Alice (who is not an owner) 0.5 Ethers for Project Management services";
