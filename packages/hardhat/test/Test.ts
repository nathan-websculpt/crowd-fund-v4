...
describe("🛠️🛠️  Multisig Crowd Fund Test  🛠️🛠️", function () {
  this.timeout(125000);
  ...
  it("🚀 Should deploy CrowdFund 🚀", async function () {
    ...
    console.log("\n✅ deployed CrowdFund at address: ", crowdFund.address);
  });

  it("🤜🤛 Contract should transfer ownership upon deployment", async function () {
    await expect(await crowdFund.owner()).to.equal(owner.address);
    console.log("✅ Contract Owner transferred successfully to: ", owner.address);
  });

  describe("\n\n⛓️ Making test Fund Runs", function () {
  ...
  describe("\n😎 Making Proposals", function () {
    ...
    it("(2-sig wallet)...John proposes to pay Bob (who is not an owner) 0.75 Ethers", async function () {
      ...
      console.log("\n\t📌 Bob has a balance of: ", formatEther(bobFirstBalance));
    ...
    it("(3-sig wallet)...Ross proposes to pay Alice 0.5 Ethers", async function () {
      ...
      console.log("\n\t📌 Alice has a balance of: ", formatEther(aliceFirstBalance));
